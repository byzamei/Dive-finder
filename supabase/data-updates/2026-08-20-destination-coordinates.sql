...
select 'destination', d.id, 'coordinates',
  jsonb_build_object('latitude', d.latitude, 'longitude', d.longitude),
  src.id, 'editorial', now(), 'medium', 'verified', false
from destinations d, src
where d.slug in (
  'maldives', 'raja-ampat', 'komodo', 'malapascua', 'galapagos', 'red-sea-egypt', 'socorro', 'cozumel',
  'bonaire', 'sipadan', 'palau', 'fiji', 'great-barrier-reef', 'mozambique', 'south-africa-aliwal-sodwana',
  'azores', 'madeira', 'french-polynesia', 'bali-nusa-penida', 'coron'
)
and not exists (
  select 1 from data_claims c
  where c.entity_type = 'destination' and c.entity_id = d.id and c.field_name = 'coordinates'
);
