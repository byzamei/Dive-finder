...
export function formatDestinationPrice(price: DestinationStartingPrice): string {
...
export function FilteredExplorer({ initialCriteria }: { initialCriteria: SearchCriteria }) {
  ...
  const pins: MapPin[] = useMemo(() => {
    ...
    return base.map((pin) => {
      ...
      return price ? { ...pin, priceLabel: formatDestinationPrice(price) } : pin;
    });
  }, [ranked, prices]);
  ...
  return (
    <div className="mt-6 flex flex-col gap-6 lg:flex-row">
      <div className="lg:w-[420px] lg:shrink-0">
        <FiltersPanel criteria={criteria} species={species} update={update} toggleInArray={toggleInArray} />
            ...
            return (
              <div key={d.id} className="rounded-xl2 border border-abyss-100 bg-white p-4 shadow-card">
                <Link href={`/destinations/${d.slug}`} className="focus-ring flex items-start justify-between gap-3">
                  ...
                  {price && <p className="shrink-0 font-display text-lg text-ocean-700">{formatDestinationPrice(price)}</p>}
                </Link>
